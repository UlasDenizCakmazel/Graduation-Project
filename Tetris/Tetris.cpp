...
int keypressed;
ICBYTES screen;
ICBYTES Blocks, Block, Logo1, Logo2, TextLogo1, TextLogo2;
HANDLE HMutex;
...
void PrintNextBlock(GameBlockType type)  {
	...
	FillRect(screen, 483, 130, 144, 110, 0x252F62);
...
void StartGame() {
	...
	Copy(TextLogo1, 1, 1, 175, 82, TextLogo2);
	PasteNon0(TextLogo2, 235, 10, screen);
	...
	while (1) {
		while (gameState != GAMEOVER) {
			...
			else if (keypressed == 40 && obj.DropState && gameState != PAUSE) // down_arrow = 40 (move down)
			{
				BlockDrop();
				keypressed = 0;
			}
...
void Loading() {
	int i = 0;
	while (screenState == INTRO) {
		i += 10;
		if (i < 250) {
			FillRect(screen, 90, 400, i * 2, 25, 0xFFFFFF);
		}
		else {
			screen = 0;
			FillRect(screen, 90, 400, i * 2, 25, 0x000000);
			screenState = GAME;
...
void DrawThread() {
	...
	while (1) {
		...
		{
		case INTRO: {
			Copy(Logo1, 1, 1, 294, 328, Logo2);
			PasteNon0(Logo2, 180,50, screen);
			CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)Loading, NULL, 0, NULL);
			break;
		}
		case MENU: {
			break;
...
void ICGUI_main() {
	...
	ReadImage("Assets/TetrisLogo.bmp", Logo1);
	ReadImage("Assets/TetrisTextLogo.bmp", TextLogo1);
